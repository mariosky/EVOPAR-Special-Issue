...
towards distributed, ubiquitous and shared computing resources, exploiting network infrastructure and what is known as the Cloud computing model\cite{}.
...
Indeed, new evolutionary models, such as the one presented with EvoSpace, allow the design of EAs that include algorithmic features present in natural evolutionary systems, but are difficult to reproduce in sequential and synchronous processes \cite{}.
...
Such a strategy can also be found in the more recent works of Secretan et al. \cite{} and Clune and Lipson \cite{}, who respectively developed web applications for the interactive
...
For instance, Cole et al. \cite{} uses the popular Berkeley Open Infrastructure for Network Computing (BOINC) to distribute an evolutionary algorithm,
...
where the evolving population of a GA is stored in a centralized pool or database. Bollini and Piastra \cite{} developed a system that decouples population storage from the evolutionary operations.
...
%We could add a comment indicating that even person could do part of the job, for instance assigning fitness in an interactive EA. (Mario)
...
Given that EvoSpace is implemented as a Python module, a simple way to implement the EvoWorkers is with the DEAP
(Distributed Evolutionary Algorithms in Python) library \cite{DEAP_JMLR2012}, using the basic non-distributed GA version.
...
(for instance processes used for database systems and task queues).
