...
The EvoSpace Model has two main components: A set of EvoWorkers and a single instance of an EvoStore. An EvoStore manages a set of objects representing individuals in a EA population. As it happens in the basic PEA described earlier, EvoWorkers pull a subset of individuals from the EvoStore, as in a tuple space, pulled individuals are removed from the EvoStore and are not available for other workers. There are two differences with the pull operation in a tuple-space. The first is that individuals removed from the EvoSpace are a random subset or sample from the population, this differs from the tuple-space paradigm which removes only those tuples matching a pattern (query).   

on which they perform a partial evolutionary process, and then return the newly evolved subpopulation to the central store where the new individuals are reinserted, at which point they can be taken by other clients. 
...
Indeed, new evolutionary models, such as the one presented with EvoSpace, allows the design of PEAs that include algorithmic features present in natural evolutionary systems, but are difficult to reproduce in sequential and synchronous processes \cite{eiben}.
...
%Menionar el modelo 
%Se describe una implemantación real
