...
\begin{abstract}
This work presents the EvoSpace model, a framework for the development of pool-based evolutionary algorithms (Pool-EA).
...
and adding additional features to tackle some of the issues associated with pool-based evolutionary algorithms;
...
Particularly, the problems of lost connections and system parametrization are studied and validated experimentally, with encouraging results,
...
The basic idea behind Pool-based EAs (Pool-EAs) is that all the participants of the distributed algorithm share a common population stored in a central server, from which each participant can pull a set of tasks or data from the server, perform some work, and then return the results back to the server. Pool-EAs differ from the closely related island-based system, mainly with regards to the responsibilities that are assigned to the server. In an island-based system the server is usually responsible for keeping track of the migration of individuals from one island to another. The server must know in advance which are the available participants in order to send the immigrant individuals.
In Pool-EAs, on the other hand, the server only receives stateless requests from participants in a pull based interaction. 
...
\begin{itemize}
	...
	\item Starvation of the Population Pool: this happens when there are no more tasks or data in the server, participants must wait for tasks or data to be available,
	which waists available computational resources.
	...
	\item Participants are unreliable: lost work can happen when participants abandon the work, malfunction, or loose their connection, thus delaying or failing to send the results back to the server.
...
Issue (5) is a deployment problem, that can be easily addressed at the technological level.
On the other hand, most of the other issues have been extensively studied in distributed systems research.
...
ESM is built conceptually around a central repository or population store, incorporating some of the main principles of the tuple-space model described above.
However, the ESM is not simply an implementation of a=the tuple-space model for EAs, since
it incorporates additional mechanisms and concepts specifically for the development of Pool-EAs.
The ESM was developed to address each of the above listed issues for Pool-EAs.
...
conceptual and implementation levels, discussing how the model can be employed to implement a Pool-EA.
...
Three experimental validations are performed in Section \ref{sec:exp1} using genetic algorithm (GA) benchmarks:
first, the ESM is compared with other distributed EAs to illustrate some of its
...
In an IMEA, each island represents an individual evolutionary process, sharing some of the same dynamics as standard EAs, which periodically exchange
individuals following a synchronized and coordinated process.
Therefore, an IMEA presents a fixed topological structure, with a predefined interaction protocol among each of the evolving populations.
On the other hand, a Pool-EA does not employ such a coordinated interaction scheme, workers perform an isolated evolutionary process using a sample of individuals
taken from the central pool.
...
in the experimental section of the present paper.
...
However, the most complete Pool-EA model proposed thus far is the SofEA algorithm by Merelo et al. \cite{sofea1,sofea2,sofea3}.
...
For instance, when once an individual is evaluated in SofEA, its fitness cannot be reassigned based on a future evaluation,
this limitation is particularly important in the development of interactive systems where several users can provide input regarding the quality of a solution \cite{Musart}.
...
In particular, two different versions of EvoSpace-py are evaluated, a local multi-process implementation and a cloud-based implementation, as described in the
previous section.
The goal is to evaluate the performance of the ESM on search and optimization problems using the OneMax, K-trap problem \cite{trap} and P-Peaks function \cite{Jong:PS97}.
In particular, the evaluation focuses on the quality of the results found, the computational effort, and the performance of ESM with different numbers
of connected EvoWorkers, comparing with an IMEA and a canonical GA implementation.
...
These experiments focus on comparing EvoSpace with an IMEA and on the classic OneMax problem.
In particular, the goal is to evaluate search efficiency and computational effort of each of these strategies.
To do so, a 128-bit OneMax problem is chosen and a binary-coded GA is used in all cases, with one point mutation and two point crossover,
additional configuration details are given in Table \ref{tab:exp3}, the population is set to the minimum size that guarantees that
...
and for the IMEA the same number of demes or populations are used; in each case a different population (or deme) size is used, or in 
...
and total time to solution.
Figures \ref{fig:island}(a) and \ref{fig:island}(b) respectively show the total number of evaluations for EvoSpace and IMEA,
and Figures \ref{fig:island}(c) and \ref{fig:island}(d)  show the same for total time to solution.
First, it is clearly shown that EvoSpace performs a much more efficient search that the IMEA, requiring about half the total number of fitness evaluations 
in all cases. Moreover, EvoSpace the number of fitness evaluations seems to be independent of the number of workers that participate in the search,
in all cases achieving a median numer of evaluations below 20 thousand.
On the other hand, IMEA shows greater variability, with median values varying from around 35k to as much as 50k depending on the number of
demes, without a clear trend.

On the other hand, regarding time to solution, it seems clear that with more workers EvoSpace performs better, with median results of 2.3s with 2 workers
down to 1.5 with six.
The IMEA however varies between 0.7s and 1.1s, without any clear improvement as the number of demes increases.
These results are expected, given that the IMEA is executed locally within the same DEAP process, while EvoSpace is incurring a communication overhead between Redis,
JSON and DEAP.
...
\begin{figure*}[t]
    ...
    \subfigure[EvoSpace]
    {
       \includegraphics[width=5.5cm]{one_max_plot_evaluations_evospace.eps}
       %\includegraphics[width=5cm]{Plot-Individuals-Eval-k4w32s32.eps}

    }
    \subfigure[IMEA]
    {
        \includegraphics[width=5.5cm]{one_max_plot_evaluations_island.eps}
    }
    \\
    \subfigure[EvoSpace]
    {
        \includegraphics[width=5.5cm]{one_max_plot_time_evospace.eps}
    }
    \subfigure[IMEA]
    {
        \includegraphics[width=5.5cm]{one_max_plot_time_island.eps}
    }
    \caption{
...
Hence, the goal of this section is to evaluate the effect that the reinsertion algorithm has on the total
...
Nevertheless, these results suggest that a random heterogeneous parametrization could be used as a simple parametrization approach for the EvoSpace model.
...
Second, performance of the ESM is equivalent or better than standard search, with the added benefit of improved efficiency and population diversity during the search.
Third, experimental results illustrate the benefits of adding client workers to the evolutionary process.
