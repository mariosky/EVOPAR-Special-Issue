...
\begin{abstract}
This work presents the EvoSpace model, a framework for the development of pool-based evolutionary algorithms.
...
that suggest how EvoSpace can be used to develop and implement different PEAs for search and optimization.    
...
This paper is part of an ever-growing effort in the field of EC to develop algorithms following an opportunistic approach to computing, allowing the exploitation of freely available services over the Internet or volunteer computing resources. To enable this arbitrary plug-in and plug-out of computational resources for distributed EAs, a new pool-based architecture has been proposed \cite{sofea1,sofea2,PoolvsIsland}. The basic idea behind Pool-based EAs (PEAs) is that all the participants of the distributed algorithm share a common population stored in a central server, from which participants pull a set of tasks or data from the server, perform some work and then return the results back to the server. PEAs differ from an island-based system,
...
\begin{itemize}
	\item Work Redundancy: in a PEA, two or more participants often take the same tasks or data to work on, thus wasting resources.
...
The conceptual model described above is the core framework of the ESM. Additional components must be defined in order to implement a complete PEA, but these will be constrained by the structure and functionality of this core. The next layer of components has to be designed in accordance with the particular type of PEA to be implemented. Nonetheless, these are still abstract components, not tied to a particular implementation platform. In the next section, the main layer of components required to implement a EA are presented. 
...
However, as stated before, there are possible downsides to using a PEA, some of which are directly addressed my the underlying ESM.
...
and the tedious problem of algorithm parametrization, a common issue with almost all EAs that is severely amplified in a PEA approach.

\subsection{Unreliable Workers}
In this section, the effect of node unavailability in an EvoSpace PEA is assessed.
...
good results.
Therefore the goal of this section is to evaluate RPSS on an EvoSpace PEA, which lacks the fixed population structure of an IMEA.
...
PEAs present several interesting properties that are not present in standard EAs, storing the population in a central store and
...
In this sense, what is particularly interesting of a PEA is that it incorporates features from natural evolution that are abstracted away in common EA implementations.
...
However, PEAs also present several noteworthy design challenges, that must be addressed.
...
At the implementation level, this work also presents an ESM instantiation called EvoSpace-py, a software tool that simplifies the deployment of PEAs,
as multi-threaded processes or on the cloud.
The system programmed using the Python programming language and the well-known DEAP library for EAs, additionally the ESM model is implemented using
...
Third, several apparent issues with the PEA approach are studied, regarding the lost connection of unreliable workers and the increases size of the algorithm's parameter space.
In both cases, experimental results suggest that the ESM can handle both issues quite robustly, using inbuilt mechanism and simple implementation strategies.
...
in order to define the proper domain of competence of PEAs in general and the ESM in particular.
Moreover, the ESM will be leveraged to implement and deploy a complete web-based PEA, that simplifies the use for other researchers in an easy to setup manner.
