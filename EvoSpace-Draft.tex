...
On the other hand, such systems present several noteworthy challenges, such as the need to manage heterogeneous and unreliable computing resources, deal with lost connections,
low bandwidth communications, abandoned work, security and privacy issues. 
 
This paper is part of an ever-growing effort in the field of EC to develop algorithms suited for taking an opportunistic approach to computing, allowing the exploitation of services freely available over the Internet or as volunteer computing resources. To enable this arbitrary plug-in and plug-out of computational resources for distributed EAs, a new pool-based architecture has been proposed \cite{PoolvsIsland}. The basic idea behind Pool-based EAs (PEAs) is that all the participants of the distributed algorithm share a common population stored in a central server, from which participants pull a set of tasks or data from the server, perform some work and then return the results back to the server. PEAs differs from an island-based system, for instance, since the server only receives stateless requests from participants in a pull based interaction.
...
%In this section the ESM is presented in detail.
...
%In summary, EvoWorkers extract a small subset of the population, and use it as the initial population for a short evolutionary algorithm that is executed locally on the client machine. Afterwards, the evolved population from each EvoWorker is returned and reinserted into the EvoSpace container.
...
An EvoStore provides a set of interface methods that operate over a set of objects $ES_i$, each of which can be seen as an independent EA population. For an EA system, objects represent individuals, these are defined in a later subsection. Objects can be removed and replaced into $ES_i$ using a specified set of methods. However, the ESM is different from other tuple space implementations in the sense that retrieving and reading objects from ES are defined as random operations. Individual objects are not of high interest when accessing an $ES$, instead population samples are desired. Therefore, an EvoStore offers the following interface methods:
...
These methods can be used to implement different PEAs, have properties 
...
EvoWorkers are autonomous computational entities executed on client machines, they only communicate with the EvoStore and not woth each other. Communication is carried out through message passing see Algorithm \ref{alg:worker}. Each EvoWorker has local algorithm that executes the main code of the EA. The \textbf{EvoWorker} process is straightforward, it requests a set of objects $A_i$ from the $ES_i$ set.
Afterwards, localy the $Evolve()$ function is called where the actual evolutionary cycle is performed. In this scenario, $A_i$ can be seen as a local population on which evolution is carried out for $g$ generations. The result of this evolution is then returned and reinserted into $ES_i$, then the EvoWorker can request a new set and repeat the process. Otherwise, each EvoWorker could specialize on a particular part of the evolutionary process, such as selection, evaluation or genetic variation.
