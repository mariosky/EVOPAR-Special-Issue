...
Moreover, the \textbf{ReInsertionManager} can also make a $ReInsert(i)$ call when an $EvoWorker$ is assumed to have lost a connection with the server,
after waiting for a maximum amount of time for a return, thus recovering the population sample taken by the EvoWorker.
...
using with one-point crossover (with crossover probability set to 1) and one-point mutation (mutation probability set to 0.06).
The number of individuals in the EvoStore is set to 1024 for 4-trap experiments and to 4096 for 5-trap,
and the maximum number of total samples that can be taken from the EvoStore in each run is 1000.
...
while otherwise using basic representation and genetic operators.
...
There seems to be a point at which the overall speedup of the distributed search levels off,
at least with the problems tested here.
% This is
